...
public class CheckoutActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (subTotal >= 5000) {
            ...
            totalTextView.setText("$ " + (subTotal + 10));
        }
            ...
            .addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    if(task.isSuccessful() && task.getResult() != null) {
                        Object lastOrderIdObj = task.getResult().get("lastOrderId");
                        Object countOfOrderedItemsObj = task.getResult().get("countOfOrderedItems");
                        Object priceOfOrdersObj = task.getResult().get("priceOfOrders");

                        // Kiểm tra null trước khi ép kiểu
                        prevOrderId[0] = lastOrderIdObj != null ? ((Number)lastOrderIdObj).intValue() : 0;
                        countOfOrderedItems[0] = countOfOrderedItemsObj != null ? ((Number)countOfOrderedItemsObj).intValue() : 0;
                        priceOfOrders[0] = priceOfOrdersObj != null ? ((Number)priceOfOrdersObj).intValue() : 0;

                        // Tiếp tục gọi lấy cart items
                                    ...
                                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                        if (task.isSuccessful()) {
                                            for (QueryDocumentSnapshot document : task.getResult()) {
                                                ...
                                                int price = priceObj != null ? ((Number)priceObj).intValue() : 0;
                                                int quantity = quantityObj != null ? ((Number)quantityObj).intValue() : 0;

                                                productPrice[0].add(price);
                                                productQuantity[0].add(quantity);

                                                OrderItemModel item = new OrderItemModel(prevOrderId[0] + 1,
                                                        ((Number)document.get("productId")).intValue(),
                        ...
                        // Xử lý lỗi khi không lấy được details
                        Toast.makeText(CheckoutActivity.this, "Failed to load order details.", Toast.LENGTH_SHORT).show();
                    }
                }
            });
}
